...
var menuPosition = "0px,0px",
  showMenu = false;
let vh = [
  (minLvl = 1),
  (maxLvl = 500),
  (valuePlayersMessage = 7),
  (refreshPlayer = 30),
  (clan = "link klanu"),
  (World = "katahha"),
  (dcUrl = "Link DC"),
...
`;
  ...
  hMenu.classList.add("newWindowStyle");
  ...
  settingsBox.addEventListener("click", () => {
    if (hMenu.style.display == "block") {
      ...
      globalBox.style.minWidth = "30px";
      globalBox.style.lineHeight = '0';
      ...
      const y = parseInt(menuPosition.split(",")[1]) + 33;

      
      hMenu.style.left = x;
      hMenu.style.top = `${y}px`;

      CPO.style.display = "none";
      CPO.style.height = "100px";

      globalBox.style.height = "0px";
      globalBox.style.minWidth = "246px";
      globalBox.style.lineHeight = '1px';
    }
  });
    ...
    GM_xmlhttpRequest({
      ...
      onload: function (response) {
        ...
        var p = s.querySelectorAll(
          "div.guild-members-container table tbody tr",
        );
  ...
  async function sendDiscordMessage(title, a) {
    ...
    data.append(
      ...
      JSON.stringify({
        ...
        avatar_url:
          "https://avatar.rf4game.com/rf4game.de/wp-content/uploads/avatar/256/2172/2172410.jpg",
        embeds: [
          {
            color: 1388608,
  ...
  function getOnlineMember() {
    ...
    GM_xmlhttpRequest({
      ...
      onload: function (response) {
        ...
        var s = document.createElement("div");
        s.innerHTML = v;
        var nickQ = s.querySelector(
          `div > div > div > div.body-container > div > div.overlay.current-players-overlay > div.light-brown-box.news-container.no-footer.${vh[5]}-popup > div.news-body`,
        );
